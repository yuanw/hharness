...
startRun agent runAction = do
  ...
  void $ forkIO $ do
    ...
    atomically $ modifyTVar' (_sv agent) $ \st -> st
...
mkConfig agent s =
  ...
  in AgentLoopConfig
    { alcModel               = _model s
    , alcStreamOptions       = (aoStreamOptions opts)
...
applyEvent ev s = case ev of
  ...
  EvToolExecEnd tcId _ _ isErr ->
    s { _pendingToolCalls = Set.delete tcId (_pendingToolCalls s)
      , _errorMsg = if isErr
