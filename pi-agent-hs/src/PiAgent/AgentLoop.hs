...
agentLoop
  ...
  -> AgentLoopConfig
  -> StreamFn
  -> Maybe (IO Bool)   -- ^ cancellation check: return 'True' to cancel
...
runLoop ctxRef newRef cfg streamFn cancel emit = do
  steering0 <- maybe (pure []) id (alcGetSteeringMessages cfg)
  pendingRef   <- newIORef steering0
  firstTurnRef <- newIORef True

  let outerLoop = do
        innerLoop
        followUps <- maybe (pure []) id (alcGetFollowUpMessages cfg)
          ...
          _ -> do
            ...
            if null toolCalls
              then emit (EvTurnEnd msg [])
                ...
                newSteering <- maybe (pure []) id (alcGetSteeringMessages cfg)
...
executeToolCalls
  :: AgentContext
  -> Message      -- ^ assistant message that requested these calls
  -> [ToolCall]
  -> AgentLoopConfig
  -> Maybe (IO Bool)
  -> AgentEventSink
...
executeSequential
  :: AgentContext -> Message -> [ToolCall]
  -> AgentLoopConfig -> Maybe (IO Bool) -> AgentEventSink
  -> IO [Message]
...
data CallPreparation
  = Immediate (AgentToolResult Value) Bool  -- ^ result, isError
      ...
      case atValidateArgs tool rawArgs of
        ...
        Right validArgs -> do
          blockResult <- case alcBeforeToolCall cfg of
            Nothing -> pure Nothing
            Just f  -> f $ BeforeToolCallContext
...
finalizeCall ctx assistantMsg pc (result, isErr) cfg _cancel emit = do
  (result', isErr') <- case alcAfterToolCall cfg of
    ...
    Just f  -> do
      override <- f $ AfterToolCallContext
...
mkErrResult msg = AgentToolResult
